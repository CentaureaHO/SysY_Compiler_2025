...
void Mem2Reg::InsertPhi(CFG* cfg){
    ...
    for(auto iter1:cfg->block_id_to_block){//对每个块
        for(auto iter2:iter1.second->insts){//对每条指令
            if(iter2->opcode==LLVMIR::IROpCode::LOAD){//use
    ...
    for(auto iter1:cfg->block_id_to_block){
        for(auto iter2:iter1.second->insts){
            if(iter2->opcode==LLVMIR::IROpCode::ALLOCA){
                ...
                if(ins->res->type==LLVMIR::OperandType::REG){
                    LLVMIR::RegOperand* op= (LLVMIR::RegOperand*) ins->res;
                    if(no_use_set.find(op->reg_num)!=no_use_set.end()){
                        todel[iter2]=1;
                    }
                }
            }else if(iter2->opcode==LLVMIR::IROpCode::STORE){
    ...
    //def与use在同一块
    for(auto iter:block2vset2){
        Mem2RegUseDefInSameBlock(cfg,iter.second,iter.first);
    }
    //一般情况的处理
    auto dom_tree= this->ir->DomTrees[cfg];

    for(auto iter:normal_set){//消除的alloca的对应ptr寄存器号
        ...
        while(!worklist1.empty()){
            ...
            for(auto iter2:dom_tree->dom_frontier[block_id]){//支配边界块号
                if(visited[iter2]){continue;}
                visited[iter2]=1;
                //插入phi
                LLVMIR::PhiInst* phi= new LLVMIR::PhiInst(
                    allocaid2inst[iter]->type,
                    new LLVMIR::RegOperand(++cfg->func->max_reg)
                );
...
void Mem2Reg::Rename(CFG* cfg){
    ...
    while(!worklist2.empty()){
        ...
        for(auto inst:(cfg->block_id_to_block)[block_id]->insts){//对块中每条指令
            if(inst->opcode==LLVMIR::IROpCode::ALLOCA){
                ...
                if(store_ins->ptr->type==LLVMIR::OperandType::REG){
                    ...
                    if(alloca_regs.find(ptr_reg)!=alloca_regs.end()){
                        todel[inst]=1;
                        incoming_values[block_id][ptr_reg]=incoming_values[block_id].find(store_from_reg)!=incoming_values[block_id].end()?incoming_values[block_id][store_from_reg]:store_from_reg;
                    }
                }
