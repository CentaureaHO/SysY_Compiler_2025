...
namespace Transform
{
    ...
    void UnusedFuncEliminationPass::Execute(){
        ...
        for(auto func:ir->functions){
            funcname_func_map[func->func_def->func_name]=func;
        }
        ...
        while(!func_q.empty()){
            ...
            for(auto block:func_now->blocks){
                for(auto inst:block->insts){
                    ...
                    if(to_call_func==nullptr){//调用外部的函数
        ...
        for(auto func:ir->functions){
            if(func_vis_map[func]||out_use_map[func->func_def->func_name]){
        ...
        ir->function_declare=new_function_declare;
