...
void insertAfterPHI(std::vector<Instruction*> ins_to_insert, LLVMIR::IRBlock* b) {
    ...
    while (it != b->insts.end() && (*it)->opcode == IROpCode::PHI) {
        ++it;
    }
...
std::pair<GEPInst*,Operand*> StrengthReduce::checkGEP(GEPInst* ins){
    ...
    for (unsigned int i = 0; i < ins->idxs.size(); ++i) {
        ...
        if (iter->type == OperandType::IMMEI32) {
            continue;
        }
        if (iter->type == OperandType::REG) {
            ...
            assert(0 && "Unexpected operand type in GEP index");
        }
    }
    if(invar_cnt!=1){
    ...
    if(scevexp==nullptr||scevexp->length()!=1||scevexp->operators[0]!=Analysis::CROperator::ADD){return {nullptr,nullptr};}
    ...
    if(step.type==Analysis::CROperand::Type::CONSTANT){
        step_val=step.const_val;
    }else if(step.type==Analysis::CROperand::Type::LLVM_OPERAND){
        if(step.llvm_op->type==OperandType::IMMEI32){  
            step_val=((ImmeI32Operand*)step.llvm_op)->value;
        }else{
    ...
    auto new_gep=new GEPInst(
        ...
        ins->idx_type,
        ins->base_ptr,
        new RegOperand(++loop->cfg->func->max_reg),
        ins->dims,
        ins->idxs
    );
    ...
    for(unsigned int i=iv_idx;i<ins->dims.size();i++){
        step_val*=ins->dims[i];
    }

    auto new_imm_step= new ImmeI32Operand(step_val);
...
void StrengthReduce::doGEPStrengthReduce(){
    ...
    for(auto iter:loop->loop_nodes){
        ...
        for(auto ins:iter->insts){
            ...
            RegOperand* new_gep_in_loop=new RegOperand(++loop->cfg->func->max_reg);
            RegOperand* update_op =new RegOperand(++loop->cfg->func->max_reg);

            //插入更新指令，使用gep来更新 %2 = gep %1, step 
            GEPInst* update_inst=new GEPInst(
                new_gep->type,
                new_gep->idx_type,
                new_gep_in_loop,
                update_op,
            ...
            auto new_phi=new PhiInst(
                LLVMIR::DataType::PTR,
                new_gep_in_loop,
                new std::vector<std::pair<Operand*, Operand*>>{}
            );

            new_phi->vals_for_labels.push_back(
                {
                    new_gep->GetResultOperand(),
                    new LabelOperand(loop->preheader->block_id)
                }
            );

            new_phi->vals_for_labels.push_back(
                {
                    update_inst->GetResultOperand(),
                    new LabelOperand(latch_num)
                }
            );
