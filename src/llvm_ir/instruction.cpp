...
void LoadInst::Rename(std::map<int,int> &replace){
    if(this->ptr->type==OperandType::REG){//保证是可替换的reg
        int reg=((RegOperand*)(this->ptr))->reg_num;
        if(replace.find(reg)!=replace.end()){//保证在替换表中
            ptr=new RegOperand(replace[reg]);
        }
    }

    if(this->res->type==OperandType::REG){//保证是可替换的reg
...
void StoreInst::Rename(std::map<int,int> &replace){
    ...
    if(this->val->type==OperandType::REG){//保证是可替换的reg
...
void ArithmeticInst::Rename(std::map<int,int> &replace){
    ...
    if(this->lhs->type==OperandType::REG){//保证是可替换的reg
    ...
    if(this->rhs->type==OperandType::REG){
        ...
        if(replace.find(reg)!=replace.end()){
            rhs=new RegOperand(replace[reg]);
        }
    }
}
...
void BranchCondInst::Rename(std::map<int,int> &replace){//其实这里是cmp出来的寄存器，不会被mem2reg波及
    if(this->cond->type==OperandType::REG){//保证是可替换的reg
...
std::vector<int> CallInst::GetUsedRegs(){
    ...
    for(auto iter:args){
        ...
        if(op->type==OperandType::REG){
            ret.push_back(((RegOperand*)op)->reg_num);
        }
    }
    return ret;
}
...
void CallInst::Rename(std::map<int,int> &replace){
    for(auto &iter:args){
        ...
        if(op->type==OperandType::REG){
            ...
            if(replace.find(regno)!=replace.end()){
                iter.second=new RegOperand(replace[regno]);
            }
        }
    }
}
...
std::vector<int> RetInst::GetUsedRegs(){
    ...
    if(ret!=nullptr&&ret->type==OperandType::REG){
        retvec.push_back(((RegOperand*)ret)->reg_num);
    }
...
std::vector<int> GEPInst::GetUsedRegs(){
    ...
    if(base_ptr->type==OperandType::REG){
        retvec.push_back(((RegOperand*)base_ptr)->reg_num);
    }
    for(auto iter:idxs){
        if(iter->type==OperandType::REG){
            retvec.push_back(((RegOperand*)iter)->reg_num);
        }
    }
    return retvec;
}
...
void GEPInst::Rename(std::map<int,int> &replace){
    if(base_ptr->type==OperandType::REG){
        ...
        if(replace.find(regno)!=replace.end()){
            base_ptr=new RegOperand(replace[regno]);
        }
    }
    for(auto &iter:idxs){
        if(iter->type==OperandType::REG){
            ...
            if(replace.find(regno)!=replace.end()){
                iter=new RegOperand(replace[regno]);
            }
        }
    }
}
...
int FuncDeclareInst::GetResultReg(){
    return -1;
}
...
std::vector<int> SI2FPInst::GetUsedRegs(){
    ...
    if(f_si->type==OperandType::REG){
        retvec.push_back(((RegOperand*)f_si)->reg_num);
    }
    return retvec;
}
...
void SI2FPInst::Rename(std::map<int,int> &replace){
    if(f_si->type==OperandType::REG){
        ...
        if(replace.find(regno)!=replace.end()){
            f_si=new RegOperand(replace[regno]);
        }
    }
}
...
std::vector<int> FP2SIInst::GetUsedRegs(){
    ...
    if(f_fp->type==OperandType::REG){
        retvec.push_back(((RegOperand*)f_fp)->reg_num);
    }
    return retvec;
}
...
void FP2SIInst::Rename(std::map<int,int> &replace){
    if(f_fp->type==OperandType::REG){
        ...
        if(replace.find(regno)!=replace.end()){
            f_fp=new RegOperand(replace[regno]);
        }
    }
}
...
std::vector<int> ZextInst::GetUsedRegs(){
    ...
    if(src->type==OperandType::REG){
        retvec.push_back(((RegOperand*)src)->reg_num);
    }
    return retvec;
}
...
void ZextInst::Rename(std::map<int,int> &replace){
    if(src->type==OperandType::REG){
        ...
        if(replace.find(regno)!=replace.end()){
            src=new RegOperand(replace[regno]);
        }
    }
}
...
std::vector<int> PhiInst::GetUsedRegs(){
    ...
    for(auto iter:vals_for_labels){
        if(iter.first->type==OperandType::REG){
            retvec.push_back(((RegOperand*)iter.first)->reg_num);
        }
    }
    return retvec;
}
...
void PhiInst::Rename(std::map<int,int> &replace){
    for(auto &iter:vals_for_labels){
        if(iter.first->type==OperandType::REG){
            ...
            if(replace.find(regno)!=replace.end()){
                iter.first=new RegOperand(replace[regno]);
            }
        }
    }
}
